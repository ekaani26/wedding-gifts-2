...
jQuery(document).ready(function ($) {
  ...
  $('#quote-form').on('submit', function (e) {
    ...
    var msgElement = form.find('#quote-msg');   // Message display element
    var submitBtn = form.find('#quote-submit'); // Submit button
    ...
    $.ajax({
      url: '../../assets/php/quote.php', // Server-side order handler
      type: 'post',                     // HTTP method
      data: form.serialize(),           // Serialize form data

      success: function (result) {
        msgElement.html(result).fadeIn();          // Show success message
        submitBtn.html('Get Quote Now').attr('disabled', false); // Reset button
